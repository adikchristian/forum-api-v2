...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async getReplyByThreadId(commentId) {
    const query = {
      // text: `'SELECT replies.*, users.username as username FROM replies INNER JOIN users ON replies.owner = users.id WHERE replies.comment_id = $1 ORDER BY replies.date'`,
